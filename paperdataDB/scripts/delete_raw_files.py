...
def delete_files(usrnm, pswd, confirm, failed_delete):
	...
	cursor.execute('SELECT obsnum, raw_path, tape_index, delete_file from paperdata where delete_file = 1 order by julian_date asc')
	...
	for items in results:
		...
		tape_path = items[2]
		delete = int(items[3])
		if delete == 1 and tape_path != 'NULL' and '2015' in tape_path and raw_path != 'NULL':
